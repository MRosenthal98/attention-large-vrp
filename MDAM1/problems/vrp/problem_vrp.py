...
class CVRP(object):
    ...
    def get_costs(dataset, pi):
        ...
        distance = (d.squeeze(0)[1:] - d.squeeze(0)[:-1]).norm(p=2, dim=1).sum(0) + (d.squeeze(0)[0] - dataset['depot'].squeeze(0)).norm(p=2, dim=0) + (d.squeeze(0)[-1] - dataset['depot'].squeeze(0)).norm(p=2, dim=0)
        # print(distanc

        # file_exists = os.path.isfile(opts.pi_file)

        # with open(opts.pi_file, 'w', newline='') as csvfile:
        #   writer = csv.writer(csvfile)
          
        #   if not file_exists:
        #     writer.writerows([pi.tolist()])
