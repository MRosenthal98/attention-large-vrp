...
class CVRP(object):
    ...
    def get_costs(dataset, pi):
        ...
        return (
            (d.squeeze(0)[1:] - d.squeeze(0)[:-1]).norm(p=2, dim=1).sum(0)
            + (d.squeeze(0)[0] - dataset['depot'].squeeze(0)).norm(p=2, dim=0)  # Depot to first
             + (d.squeeze(0)[-1] - dataset['depot'].squeeze(0)).norm(p=2, dim=0)  # Last to depot, will be 0 if depot is last
        ), None
    ...
    def __init__(self, filename=None, size=50, num_samples=1000000, offset=0, distribution=None):
        ...
        if filename is not None:
            ...
            with open(filename, 'rb') as f:
                data = pickle.load(f)
            self.data = [make_instance(data)]
            # self.data = [make_instance(args) for args in data[offset:offset+num_samples]] # This one is for the test

            # Not sure about this
            # with open(filename, 'rb') as f:
            #     data = pickle.load(f)
            # for cluster in data:
            #   self.data.append(make_instance(data['cluster']))
